...
# Решение
Точкой входа в прорамму является метод main в классе Application.
1. Сначала создается объект класса Subscribers, который представляет собой множество уникальных абонентов. Генерация случаных номеров происходит во время создания объекта, количество абонентов случаное до 100.
2. Создается объект класса CDR, у которого вывывается метод generateCDRFile(), где входным параметром является множество номеров. В результате в папке CDRFile, появляется множество txt файлов.
3. Создается объект UDRgenerator и с помощью его метода generateReport(). На основе ранее сгенерированных txt файлов, в папка reports заполняется json файлами для каждого абонента в определеенный месяц. 
